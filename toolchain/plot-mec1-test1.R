...
# ###### Plot results #######################################################
plotPETotalUsedCapacity <- function(name)
{
   # ====== Plot as PDF file ================================================
   ...
   cairo_pdf(paste(sep="", name, "-CalcAppPETotalUsedCapacity.pdf"),
            width=12, height=4.5, family="Helvetica", pointsize=22)

   title <- ""

   # ====== Calculate utilisation ===========================================
   utilization <- data.frame(utilization = 100.0 * calcAppPETotalUsedCapacity$lan.calcAppPoolElementArray.calcAppServer.CalcAppPETotalUsedCapacity /
   ...
   calcAppPETotalUsedCapacity <- cbind(calcAppPETotalUsedCapacity, utilization)
   ...
   summarised <- calcAppPETotalUsedCapacity %>%
                    mutate(lan.calcAppPoolElementArray.calcAppServer.CalcAppPETotalUsedCapacity = lan.calcAppPoolElementArray.calcAppServer.CalcAppPETotalUsedCapacity / 1000000) %>%
                    ...
                    summarise(#.groups = "keep",   # Keep the grouping as is. Otherwise, it would drop the last one!
                              MeanCalcAppPETotalUsedCapacity = mean(lan.calcAppPoolElementArray.calcAppServer.CalcAppPETotalUsedCapacity),
                              MinCalcAppPETotalUsedCapacity  = min(lan.calcAppPoolElementArray.calcAppServer.CalcAppPETotalUsedCapacity),
                              MaxCalcAppPETotalUsedCapacity  = max(lan.calcAppPoolElementArray.calcAppServer.CalcAppPETotalUsedCapacity),
                              Q10CalcAppPETotalUsedCapacity  = quantile(lan.calcAppPoolElementArray.calcAppServer.CalcAppPETotalUsedCapacity, 0.10),
                              Q90CalcAppPETotalUsedCapacity  = quantile(lan.calcAppPoolElementArray.calcAppServer.CalcAppPETotalUsedCapacity, 0.90))
   print(summarised)
   ...
   p <- ggplot(summarised,
               aes(x = scenarioNumberOfCalcAppPoolUsersVariable,
                   y = MeanCalcAppPETotalUsedCapacity,
...
#                panel.background  = element_rect(fill = paste(sep="", "#", colorCU), color=bgColor, size=2),
         ...
         labs(title = title,
               x     = "Number of Pool User Instances on the UE [1]",
               y     = "Used Calculations [Calcs]") +
...
#          facet_wrap(~calcAppPoolElementSelectionPolicy) +
         ...
         geom_errorbar(aes(ymin = MinCalcAppPETotalUsedCapacity, ymax = MaxCalcAppPETotalUsedCapacity, color=lan),
                        size=1.5, width=.5) +
         geom_errorbar(aes(ymin = Q10CalcAppPETotalUsedCapacity, ymax = Q90CalcAppPETotalUsedCapacity, color=lan),
                        size=0.5, width=.25)
         geom_ribbon(aes(ymin = Q10CalcAppPETotalUsedCapacity, ymax = Q90CalcAppPETotalUsedCapacity, color=lan),
...
data <- plotPETotalUsedCapacity("mec1-test1/Results")
