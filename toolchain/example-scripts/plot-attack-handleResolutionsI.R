...
# ------ Plots --------------------------------------------------------------
filter <- "data1$attackReportUnreachableProbability == 0.0"
# "((data1$calcAppPoolElementSelectionPolicy != \"RoundRobin\") & (data1$calcAppPoolElementSelectionPolicy != \"Random\")) | (data1$attackerAttackInterval >= 0.5)"
...
plotConfigurations <- list(
   ...
   list(simulationDirectory, paste(sep="", simulationDirectory, "-Utilization.pdf"),
        ...
        "calcAppPoolElementSelectionPolicy", "attackReportUnreachableProbability", "",
         "", "", "", filter),

   list(simulationDirectory, paste(sep="", simulationDirectory, "-HandlingSpeed.pdf"),
        "User's Perspective", NA, list(seq(0,70,10)), list(1,0.7),
